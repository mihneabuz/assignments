...
echo -e "Coding style will be checked, for now 20p"
TOTAL=$(echo "$TOTAL + 20" | bc -l)

ls . | grep -i "readme" &>/dev/null

if [ $? -eq 0 ]; then
	echo -e "Readme present: ok 5p"
	TOTAL=$(echo "$TOTAL + 5" | bc -l)
else
	echo -e "Readme not present: failed 0p"
fi

echo "=============="
echo "TOTAL: "$TOTAL"/150"
echo "=============="
