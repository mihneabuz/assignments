/*
 * Protocoale de comunicatii
 * Laborator 7 - TCP
 * Echo Server
 * server.c
 */

#include <sys/socket.h>
#include <netinet/in.h>
#include <arpa/inet.h>
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
#include <errno.h>
#include <string.h>
#include <sys/types.h>

#include "helpers.h"

int s;

// Primeste date de pe connfd1 si trimite mesajul receptionat pe connfd2
// la serverul de echo connfd1 == connfd2
int receive_and_send(int connfd1, int connfd2)
{
    char buf[BUFLEN];
    int bytes_send;
    int bytes_received = 0;

    // TODO 6: Receptionati un mesaj de la connfd1

    bytes_received = recv(connfd1, buf, BUFLEN, 0);

    if (bytes_received != 0) {
        fprintf(stderr, "Received: %s", buf);
    }

    // TODO 7: Timiteti mesajul la connfd2

    bytes_send = send(connfd2, buf, bytes_received, 0);
    
    if (bytes_send != 0) {
        fprintf(stderr, "Sent: %s", buf);
    }

    return bytes_received;
}

void run_echo_server(int listenfd)
{
    printf("Running echo server...\n");

    struct sockaddr_in client_addr;

    int bytes_received;
    int connfd = -1;
    socklen_t socket_len = sizeof(struct sockaddr_in);

    // TODO 4: Ascultati pentru un singur client pe socketul dat

    int r = listen(s, 1);

    if (r < 0) {
        perror("Cannot listen on socket!\n");
        exit(1);
    }

    printf("Listening fot connection!\n");

    // TODO 5: Acceptati o conexiune

    connfd = accept(s, (struct sockaddr*) &client_addr, &socket_len);

    printf("Accepted connection!\n");

    do {
        bytes_received = receive_and_send(connfd, connfd);
    } while (bytes_received > 0);

    // TODO 8: Inchideti conexiunea si socket-ul clientului
    
    close(connfd);
}

void run_chat_server(int listenfd)
{
    struct sockaddr_in client_addr1;
    struct sockaddr_in client_addr2;

    int bytes_received;
    int connfd1 = -1;
    int connfd2 = -1;
    socklen_t socket_len = sizeof(struct sockaddr_in);

    // TODO 4: Ascultati pentru doi clineti pe socketul dat
    
    int r = listen(s, 2);

    if (r < 0) {
        perror("Cannot listen on socket!\n");
        exit(1);
    }

    // TODO 5: Acceptati doua conexiuni
    
    connfd1 = accept(s, (struct sockaddr*) &client_addr1, &socket_len);
    connfd2 = accept(s, (struct sockaddr*) &client_addr2, &socket_len);

    do {
        bytes_received = receive_and_send(connfd1, connfd2);

        if (bytes_received == 0)
            break;

        bytes_received = receive_and_send(connfd2, connfd1);
    } while (bytes_received > 0);

    // TODO 8: Inchideti conexiunile si socketii creati
    
    close(connfd1);
    close(connfd2);
}

int main(int argc, char* argv[])
{
    int listenfd = -1;
    struct sockaddr_in serv_addr;
    socklen_t socket_len = sizeof(struct sockaddr_in);


    if (argc != 3) {
        printf("\n Usage: %s <ip> <port>\n", argv[0]);
        return 1;
    }

    // TODO 1: Creati un socket TCP pentru receptionarea conexiunilor
    
    s = socket(PF_INET, SOCK_STREAM, 0);

	if (s < 0) {
		perror("Failed to create socket!\n");
		return 1;
	}
    
    printf("Created socket for listen!\n");

    // TODO 2: Completati in serv_addr adresa serverului pentru bind, familia de adrese si portul rezervat pentru server

    struct sockaddr_in listen;
    listen.sin_family = AF_INET;
    listen.sin_port = htons(atoi(argv[2]));
    listen.sin_addr.s_addr = INADDR_ANY;

    // TODO 3: Asociati adresa serverului cu socketul creat folosind bind

    int r = bind(s, (struct sockaddr*) &listen, sizeof(listen));
	
    if (r < 0) {
		perror("Failed to bind socket!\n");
		return 1;
	}

    printf("Socket bound!\n");

    //run_echo_server(listenfd);
    run_chat_server(listenfd);


    // TODO 9: Inchideti socketul creat

    close(s);

    return 0;
}
