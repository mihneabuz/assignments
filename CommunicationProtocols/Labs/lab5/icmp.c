...
void ping(int sockfd, struct in_addr ip_addr, int count)
{
	...
	for (int i = 0; i < count; i++) {
		...
		struct iphdr *ip_hdr_reply = (struct iphdr *)(reply.payload + IP_OFF);
		struct icmphdr *icmp_hdr_reply = (struct icmphdr *)(reply.payload + ICMP_OFF);
	
		struct in_addr rep_ip;
		rep_ip.s_addr = ip_hdr_reply->saddr;

		if (icmp_hdr_reply->type == 0 && icmp_hdr_reply->code == 0)
			printf("GOT response from %s in %.2fms...\n", inet_ntoa(rep_ip), time_elapsed);
		else if (icmp_hdr_reply->type == 11)
			printf("TTL excedeed!\n");
...
void traceroute(int sockfd, struct in_addr ip_addr)
{
	...
	while (1) {
		...
		// check if we found new node in route
		if (last_address != ip_hdr_reply->saddr) {
			last_address = ip_hdr_reply->saddr;
			count++;

			struct in_addr rep_ip;
			rep_ip.s_addr = ip_hdr_reply->saddr;
			printf("%s%d  %s in %.2fms\n", (count < 10 ? "  " : " "),
					count, inet_ntoa(rep_ip), time_elapsed);
		}
