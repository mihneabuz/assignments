...
void get_ip(char* name)
{
	...
	for (p = result; p != NULL; p = p->ai_next) {
		if (p->ai_family == AF_INET) {
			...
			inet_ntop(AF_INET, &entry->sin_addr, buffer, sizeof(buffer));
			printf("IPv4: %s  Port: %d  CanonName: %s  Protocol: %d\n",
					buffer, entry->sin_port, p->ai_canonname, p->ai_protocol);
		} else {
			struct sockaddr_in6* entry = (struct sockaddr_in6*) p->ai_addr;
			inet_ntop(AF_INET6, &entry->sin6_addr, buffer, sizeof(buffer));
			printf("IPv6: %s  Port: %d  CanonName: %s  Protocol: %d\n",
					buffer, entry->sin6_port, p->ai_canonname, p->ai_protocol);
		}
	}
...
void get_name(char* ip)
{
	...
	ret = getnameinfo((struct sockaddr*) &addr, sizeof(addr), host, sizeof(host),
